...
//#region ../../node_modules/.pnpm/maverick.js@0.44.1/node_modules/maverick.js/dist/server/signal.js
...
function serverEffect(effect, options) {
	if (typeof process !== "undefined" && process.env?.NODE_ENV === "test") return effect$1(effect, options);
	return noop;
}
const EVENT = class Event {}, DOM_EVENT = Symbol("DOM_EVENT");
...
//#endregion
//#region ../../node_modules/.pnpm/maverick.js@0.44.1/node_modules/maverick.js/dist/server/string.js
/**
...
//#endregion
//#region ../../node_modules/.pnpm/maverick.js@0.44.1/node_modules/maverick.js/dist/server/std.js
...
//#endregion
//#region ../../node_modules/.pnpm/maverick.js@0.44.1/node_modules/maverick.js/dist/server/controller.js
...
var Instance = class {
	...
	#attachAttrs() {
		...
		for (const name of Object.keys(this.attrs)) setAttribute(this.el, name, unwrapDeep.call(this.component, this.attrs[name]));
	}
	#attachStyles() {
		if (!this.styles) return;
		for (const name of Object.keys(this.styles)) setStyle(this.el, name, unwrapDeep.call(this.component, this.styles[name]));
	}
	#setAttr(name) {
...
//#endregion
//#region ../../node_modules/.pnpm/maverick.js@0.44.1/node_modules/maverick.js/dist/server/index.js
...
//#endregion
//#region ../../node_modules/.pnpm/maverick.js@0.44.1/node_modules/maverick.js/dist/server/symbols.js
const SETUP = /* @__PURE__ */ Symbol(0);
//#endregion
//#region ../../node_modules/.pnpm/maverick.js@0.44.1/node_modules/maverick.js/dist/server/react.js
...
function escape(value, isAttr = false) {
	...
	let left = 0, out = "";
	while (iDelimeter >= 0 && isAmpersand >= 0) if (iDelimeter < isAmpersand) {
...
var MaverickServerElement = class {
	...
	setup() {
		const instance = this.$.$$;
		scoped(() => {
			if (this.hasAttribute("class")) parseClassAttr(this.classList.tokens, this.getAttribute("class"));
			if (this.hasAttribute("style")) parseStyleAttr(this.style.tokens, this.getAttribute("style"));
			instance.setup();
			instance.attach(this);
			if (this.classList.length > 0) this.setAttribute("class", this.classList.toString());
			if (this.style.length > 0) this.setAttribute("style", this.style.toString());
			if (this.keepAlive) this.setAttribute("keep-alive", "");
		}, instance.scope);
	}
	getAttribute(name) {
...
var ServerAttributes = class {
	...
	toString() {
		...
		let result = "";
		for (const [name, value] of this.#tokens) result += ` ${name}="${escape(value, true)}"`;
		return result;
	}
};
var ServerStyle = class {
	...
	toString() {
		if (this.#tokens.size === 0) return "";
		let result = "";
		for (const [name, value] of this.#tokens) result += `${name}: ${value};`;
		return result;
	}
};
...
function createServerComponent(Component, options) {
	function ServerComponent(props) {
		let component = createComponent(Component, {
			props,
			scope: React.useContext(ReactScopeContext).current
		}), host = new MaverickServerElement(component), attrs = {}, { style = {}, children, forwardRef, ...renderProps } = props;
		if (options.props.size) {
			for (const prop of Object.keys(renderProps)) if (!options.props.has(prop)) attrs[prop] = renderProps[prop];
		} else attrs = renderProps;
		host.setup();
		if (host.hasAttribute("style")) {
			for (const [name, value] of host.style.tokens) style[name.startsWith("--") ? name : kebabToCamelCase(name)] = value;
			host.removeAttribute("style");
		}
		for (const [attrName, attrValue] of host.attributes.tokens) {
			const propName = attrsToProps[attrName];
			if (propName) {
				if (!(propName in attrs)) attrs[propName] = attrValue;
				host.removeAttribute(attrName);
			}
		}
		return WithScope({ current: component.$$.scope }, isFunction(children) ? children?.({
			...Object.fromEntries(host.attributes.tokens),
			...attrs,
			style
		}, component) : children, React.createElement(() => {
			host.destroy();
			return null;
		}));
	}
...
function createReactComponent(Component, options) {
	return createServerComponent(Component, { props: new Set(Object.keys(Component.props || {})) });
}
//#endregion
