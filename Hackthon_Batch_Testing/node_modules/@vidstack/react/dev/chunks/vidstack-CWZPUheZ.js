...
import { __exportAll } from "./vidstack-BLEs9rMG.js";
import * as React from "react";
//#region ../../node_modules/.pnpm/maverick.js@0.44.1/node_modules/maverick.js/dist/dev/signal.js
...
function runEffects() {
	...
	runningEffects = true;
	for (let i = 0; i < effects.length; i++) if (effects[i].$st !== STATE_CLEAN) runTop(effects[i]);
	effects = [];
	scheduledEffects = false;
...
function runTop(node) {
	let ancestors = [node];
	while (node = node[SCOPE]) if (node.$e && node.$st !== STATE_CLEAN) ancestors.push(node);
...
function onDispose(disposable) {
	...
	if (!node.$d) node.$d = disposable;
	else if (Array.isArray(node.$d)) node.$d.push(disposable);
	else node.$d = [node.$d, disposable];
	return function removeDispose() {
		if (node.$st === STATE_DISPOSED) return;
		disposable.call(null);
		if (isFunction$1(node.$d)) node.$d = null;
		else if (Array.isArray(node.$d)) node.$d.splice(node.$d.indexOf(disposable), 1);
	};
}
function dispose(self = true) {
	if (this.$st === STATE_DISPOSED) return;
	if (this.$h) if (Array.isArray(this.$h)) for (let i = this.$h.length - 1; i >= 0; i--) dispose.call(this.$h[i]);
	else dispose.call(this.$h);
	if (self) {
		const parent = this[SCOPE];
		if (parent) if (Array.isArray(parent.$h)) parent.$h.splice(parent.$h.indexOf(this), 1);
		else parent.$h = null;
		disposeNode(this);
	}
}
function disposeNode(node) {
	...
	if (node.$d) emptyDisposal(node);
	if (node.$s) removeSourceObservers(node, 0);
	node[SCOPE] = null;
	node.$s = null;
	node.$o = null;
	node.$h = null;
...
function emptyDisposal(scope) {
	try {
		if (Array.isArray(scope.$d)) for (let i = scope.$d.length - 1; i >= 0; i--) {
...
function read() {
	if (this.$st === STATE_DISPOSED) return this.$v;
	if (currentObserver && !this.$e) if (!currentObservers && currentObserver.$s && currentObserver.$s[currentObserversIndex] == this) currentObserversIndex++;
	else if (!currentObservers) currentObservers = [this];
	else currentObservers.push(this);
	if (this.$c) updateCheck(this);
	return this.$v;
}
function write(newValue) {
	...
	if (this.$ch(this.$v, value)) {
		this.$v = value;
		if (this.$o) for (let i = 0; i < this.$o.length; i++) notify(this.$o[i], STATE_DIRTY);
	}
	return this.$v;
}
...
ScopeProto.append = function(child) {
	child[SCOPE] = this;
	if (!this.$h) this.$h = child;
	else if (Array.isArray(this.$h)) this.$h.push(child);
	else this.$h = [this.$h, child];
	child.$cx = child.$cx === defaultContext ? this.$cx : {
		...this.$cx,
		...child.$cx
	};
	if (this.$eh) child.$eh = !child.$eh ? this.$eh : [...child.$eh, ...this.$eh];
};
...
const ComputeNode = function Computation(initialValue, compute2, options) {
	...
	this.$o = null;
	this.$v = initialValue;
	if (compute2) this.$c = compute2;
	if (options && options.dirty) this.$ch = options.dirty;
};
...
function updateCheck(node) {
	if (node.$st === STATE_CHECK) for (let i = 0; i < node.$s.length; i++) {
		updateCheck(node.$s[i]);
		if (node.$st === STATE_DIRTY) break;
	}
	if (node.$st === STATE_DIRTY) update(node);
	else node.$st = STATE_CLEAN;
}
function cleanup(node) {
	if (node.$h) dispose.call(node, false);
	if (node.$d) emptyDisposal(node);
...
function update(node) {
	...
	try {
		...
		if (!node.$e && node.$i) write.call(node, result);
		else {
			node.$v = result;
			node.$i = true;
		}
...
function updateObservers(node) {
	if (currentObservers) {
		if (node.$s) removeSourceObservers(node, currentObserversIndex);
		if (node.$s && currentObserversIndex > 0) {
			node.$s.length = currentObserversIndex + currentObservers.length;
			for (let i = 0; i < currentObservers.length; i++) node.$s[currentObserversIndex + i] = currentObservers[i];
		} else node.$s = currentObservers;
		let source;
		for (let i = currentObserversIndex; i < node.$s.length; i++) {
			source = node.$s[i];
			if (!source.$o) source.$o = [node];
			else source.$o.push(node);
		}
...
function notify(node, state) {
	if (node.$st >= state) return;
	if (node.$e && node.$st === STATE_CLEAN) {
		effects.push(node);
		if (!scheduledEffects) flushEffects();
	}
	node.$st = state;
	if (node.$o) for (let i = 0; i < node.$o.length; i++) notify(node.$o[i], STATE_CHECK);
}
...
function computed(compute, options) {
	const node = createComputation(options?.initial, compute, options), signal2 = read.bind(node);
	signal2[SCOPE] = true;
	return signal2;
}
...
var EventTriggers = class {
	...
	*/
	add(event) {
		...
		if (isDOMEvent(event)) this.chain.push(...event.triggers);
	}
	/**
...
function setAttribute(host, name, value) {
	if (!host) return;
	else if (!value && value !== "" && value !== 0) host.removeAttribute(name);
	else {
		const attrValue = value === true ? "" : value + "";
		if (host.getAttribute(name) !== attrValue) host.setAttribute(name, attrValue);
	}
}
/**
...
function setStyle(host, property, value) {
	if (!host) return;
	else if (!value && value !== 0) host.style.removeProperty(property);
...
//#endregion
//#region ../../node_modules/.pnpm/maverick.js@0.44.1/node_modules/maverick.js/dist/dev/string.js
/**
...
//#endregion
//#region ../../node_modules/.pnpm/maverick.js@0.44.1/node_modules/maverick.js/dist/dev/std.js
...
//#endregion
//#region ../../node_modules/.pnpm/fscreen@1.2.0/node_modules/fscreen/dist/fscreen.esm.js
var key = {
	fullscreenEnabled: 0,
	fullscreenElement: 1,
	requestFullscreen: 2,
...
//#endregion
//#region ../../node_modules/.pnpm/maverick.js@0.44.1/node_modules/maverick.js/dist/dev/controller.js
...
function provideContext(context, value, scope = getScope()) {
	if (!scope) throw Error("[maverick] attempting to provide context outside root");
	const hasProvidedValue = !isUndefined(value);
	if (!hasProvidedValue && !context.provide) throw Error("[maverick] context can not be provided without a value or `provide` function");
	setContext(context.id, hasProvidedValue ? value : context.provide?.(), scope);
}
function useContext(context) {
	const value = getContext(context.id);
	if (isUndefined(value)) throw Error("[maverick] attempting to use context without providing first");
	return value;
}
...
var Instance = class {
	...
	constructor(Component, scope, init) {
		...
		if (props) {
			...
			if (init?.props) for (const prop of Object.keys(init.props)) this.props[prop]?.set(init.props[prop]);
		}
	...
	detach() {
		...
		this.attachScope = null;
		this.connectScope = null;
		if (this.el) this.el.$$COMPONENT_NAME = null;
		this.el = null;
		this.$el.set(null);
	}
	connect() {
	...
	#attachAttrs() {
		...
		for (const name of Object.keys(this.attrs)) if (isFunction(this.attrs[name])) effect(this.#setAttr.bind(this, name));
		else setAttribute(this.el, name, this.attrs[name]);
	}
	#attachStyles() {
		if (!this.styles) return;
		for (const name of Object.keys(this.styles)) if (isFunction(this.styles[name])) effect(this.#setStyle.bind(this, name));
		else setStyle(this.el, name, this.styles[name]);
	}
	#setAttr(name) {
...
var ViewController = class extends EventTarget {
	...
	*/
	dispatch(type, ...init) {
		...
		Object.defineProperty(event, "target", { get: () => this.$$.component });
		return untrack(() => {
...
//#endregion
//#region ../../node_modules/.pnpm/maverick.js@0.44.1/node_modules/maverick.js/dist/dev/index.js
...
var State = class {
	...
	reset(record, filter) {
		for (const name of Object.keys(record)) if (!this.#descriptors[name].get && (!filter || filter(name))) record[name].set(this.record[name]);
	}
};
/**
...
//#endregion
//#region ../../node_modules/.pnpm/just-throttle@4.2.0/node_modules/just-throttle/index.mjs
...
//#endregion
//#region ../../node_modules/.pnpm/just-debounce-it@3.2.0/node_modules/just-debounce-it/index.mjs
...
//#endregion
//#region ../../node_modules/.pnpm/maverick.js@0.44.1/node_modules/maverick.js/dist/dev/react.js
...
(class extends React.Component {
	static {
		this.contextType = ReactScopeContext;
	}
	static #context;
	static #provide;
	#scope;
	constructor(props, context) {
		super(props);
		this.#scope = { current: createScope() };
...
function createClientComponent(Component, options) {
	const forwardComponent = React.forwardRef((props, forwardRef) => {
		...
		if (!stateRef.current) {
			const state = createInternalState(), component = initComponent(Component, state, props, parentScopeRef.current);
			state.component = component;
			stateRef.current = state;
			scopeRef.current = component.scope;
		}
		function onAttach() {
			let state = stateRef.current, scope = parentScopeRef.current;
			...
			if (state.el) attachToHost(state, state.el);
			if (!state.forwardRef) {
		...
		state.callbacks = {};
		for (const name of [...prevPropNames, ...newPropNames]) if (options.props.has(name)) state.component.$props[name].set(isUndefined(renderProps[name]) ? Component.props?.[name] : renderProps[name]);
		else if (options.events?.has(name) || options.eventsRE?.test(name)) state.callbacks[name] = renderProps[name];
		...
		return WithScope(scopeRef, React.createElement(AttachEffect, { effect: onAttach }), isFunction(children) ? children?.({
			...attrs,
...
function detachFromHost(state) {
	...
	if (state.el && state.listening) {
		for (const type of state.listening) state.el.removeEventListener(type, state.onDOMEvent);
...
function onDispatch(event) {
	...
	if (!callbackProp) eventTypeToCallbackName.set(event.type, callbackProp = `on${kebabToPascalCase(event.type)}`);
	this.callbacks[callbackProp]?.(...args);
}
function initComponent(Component, state, props, scope) {
...
function createReactComponent(Component, options) {
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/accessibility.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/airplay.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/arrow-down.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/arrow-left.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/arrow-up.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/chapters.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/check.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/chevron-right.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/chromecast.js
var chromecast_exports = /* @__PURE__ */ __exportAll({ default: () => Icon$24 });
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/closed-captions-on.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/closed-captions.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/download.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/eye.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/fast-backward.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/fast-forward.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/fullscreen-exit.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/fullscreen.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/music.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/mute.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/no-eye.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/odometer.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/pause.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/picture-in-picture-exit.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/picture-in-picture.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/play.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/playback-speed-circle.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/replay.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/seek-backward-10.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/seek-forward-10.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/settings-menu.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/settings.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/volume-high.js
...
//#endregion
//#region ../../node_modules/.pnpm/media-icons@1.1.5/node_modules/media-icons/dist/icons/volume-low.js
...
//#endregion
export { Component, DOMEvent, EventsController, EventsTarget, Icon$0, Icon$104, Icon$105, Icon$11, Icon$13, Icon$16, Icon$19, Icon$22, Icon$24, Icon$26, Icon$27, Icon$31, Icon$33, Icon$34, Icon$35, Icon$39, Icon$40, Icon$5, Icon$53, Icon$54, Icon$56, Icon$58, Icon$59, Icon$60, Icon$61, Icon$62, Icon$63, Icon$74, Icon$77, Icon$8, Icon$81, Icon$86, Icon$88, State, ViewController, animationFrameThrottle, appendTriggerEvent, ariaBool, camelToKebabCase, chromecast_exports, composeRefs, computed, createContext, createDisposalBin, createReactComponent, createScope, deferredPromise, effect, findTriggerEvent, fscreen, functionDebounce, functionThrottle, getScope, hasProvidedContext, hasTriggerEvent, isArray, isBoolean, isDOMNode, isFunction, isKeyboardClick, isKeyboardEvent, isMouseEvent, isNil, isNull, isNumber, isObject, isPointerEvent, isString, isTouchEvent, isUndefined, isWriteSignal, kebabToCamelCase, keysOf, listenEvent, method, noop, onDispose, peek, prop, provideContext, r, scoped, setAttribute, setStyle, signal, tick, toggleClass, untrack, uppercaseFirstChar, useContext, useReactContext, useReactScope, useSignal, useSignalRecord, useState, useStateContext, waitIdlePeriod, waitTimeout, walkTriggerEventChain, wasEnterKeyPressed };
