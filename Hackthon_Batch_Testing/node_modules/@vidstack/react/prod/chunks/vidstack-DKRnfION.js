...
var AudioProviderLoader = class {
	...
	async load(ctx) {
		...
		return new (await (import("./vidstack-CHcsWfRV.js"))).AudioProvider(this.target, ctx);
	}
};
//#endregion
...
var VideoProviderLoader = class {
	...
	async load(ctx) {
		...
		return new (await (import("./vidstack-CvezEmSQ.js").then((n) => n.provider_exports$1))).VideoProvider(this.target, ctx);
	}
};
//#endregion
...
var HLSProviderLoader = class HLSProviderLoader extends VideoProviderLoader {
	...
	async load(context) {
		...
		return new (await (import("./vidstack-DiaLSJYx.js"))).HLSProvider(this.target, context);
	}
};
//#endregion
...
var DASHProviderLoader = class DASHProviderLoader extends VideoProviderLoader {
	...
	async load(context) {
		...
		return new (await (import("./vidstack-CvezEmSQ.js").then((n) => n.provider_exports))).DASHProvider(this.target, context);
	}
};
//#endregion
...
var VimeoProviderLoader = class {
	...
	async load(ctx) {
		...
		return new (await (import("./vidstack-GkTzay8D.js"))).VimeoProvider(this.target, ctx);
	}
...
var YouTubeProviderLoader = class {
	...
	async load(ctx) {
		...
		return new (await (import("./vidstack-BmcqUMKx.js"))).YouTubeProvider(this.target, ctx);
	}
	async loadPoster(src, ctx, abort) {
	...
	async requestGoogleCast(trigger) {
		try {
			...
			if (!this.#googleCastLoader) {
				const $module = await import("./vidstack-Di0HObvT.js");
...
function throwIfAutoplayingWithReducedMotion(autoplaying) {
	...
	throw Error("[vidstack] autoplay blocked");
}
...
var MediaPlayer = class MediaPlayer extends Component {
	...
	constructor() {
		...
		context.remote = this.remoteControl = new MediaRemoteControl(void 0);
...
var SourceSelection = class {
	...
	#onSourceChange() {
		...
		const sources = $state.sources(), currentSource = peek($state.source), newSource = this.#findNewSource(currentSource, sources);
		if (sources[0]?.src && !newSource.src && !newSource.type) {
						...
						if (response.headers.get("content-type") === "application/json") {
							...
							if (isArray(json)) if (json[0] && "text" in json[0]) resolve(this.#processVTTCues(json));
								...
								for (let i = 0; i < json.length; i++) {
									...
									assert("url" in image && isString(image.url), false);
									assert("startTime" in image && isNumber(image.startTime), false);
								}
								resolve(json);
							}
