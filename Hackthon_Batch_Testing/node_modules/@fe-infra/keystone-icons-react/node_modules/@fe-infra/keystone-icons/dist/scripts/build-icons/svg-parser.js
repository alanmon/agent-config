...
export class SVGParser {
    ...
    parseMacroColors() {
        const macroColors = COLOR_ATTRIBUTES.map((attribute) => this._$(`[${attribute}]`)
            .map((_, node) => this._parseMacroValue(this._$(node).attr(attribute)))
            .toArray()
            .filter(Boolean)).flat();
        return uniqWith(macroColors, isEqual);
    }
    ...
    replaceColorWithVariables() {
        COLOR_ATTRIBUTES.forEach((attribute) => this._$(`[${attribute}]`).each((_, node) => {
            const value = this._$(node).attr(attribute);
            // Only swap currentColor and default color(#121415) to be configurable
            if (value === 'currentColor' || value === '#121415')
                this._$(node).attr(attribute, '{icon.color}');
            else {
                const macro = this._parseMacroValue(value);
                if (macro)
                    this._$(node).attr(attribute, `{this.${macro.name}}`);
            }
        }));
        return this;
    }
    replaceStrokeWidth() {
        this._$('[stroke-width]').each((_, node) => {
            ...
            if (!strokeWidthValue)
                return;
            this._$(node).attr('stroke-width', strokeWidthValue === DEFAULT_STROKE_WIDTH
                ? '{icon.strokeWidth}'
